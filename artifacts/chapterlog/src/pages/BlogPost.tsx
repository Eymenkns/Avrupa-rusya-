...
const SCHEMA_LANG: Record<string, string> = { tr: "tr", en: "en", ru: "ru", de: "de" };
...
export default function BlogPost() {
  ...
  if (!post) {
    return (
      <main className="flex-1 w-full bg-background pt-24 pb-20 flex items-center justify-center">
        <Helmet>
          <title>{t.blog.post.notFoundTitle} | ChapterLOG</title>
          <meta name="robots" content="noindex, nofollow" />
          <link rel="canonical" href={`${SITE_URL}/blog`} />
        </Helmet>
        <div className="text-center">
          <h1 className="text-2xl font-bold text-white mb-4 font-display">{t.blog.post.notFoundTitle}</h1>
          <Link href="/blog">
            <button className="inline-flex items-center gap-2 text-accent font-bold font-display">
              <ArrowLeft size={16} /> {t.blog.post.notFoundBack}
            </button>
          </Link>
        </div>
      </main>
  ...
  const articleSchema = {
    ...
    "inLanguage": SCHEMA_LANG[lang] ?? "tr",
    "author": {
      "@type": "Organization",
      "name": "ChapterLOG LLC",
      "url": SITE_URL,
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 3, "name": loc.title, "item": `${SITE_URL}/blog/${post.slug}` },
    ],
  };
  ...
  return (
    <main className="flex-1 w-full bg-background pt-24 pb-20">
      <SEO
        ...
        titleEn={`${seoEn.title} — ChapterLOG`}
        titleRu={`${seoRu.title} — ChapterLOG`}
        titleDe={`${seoDe.title} — ChapterLOG`}
        descTr={post.excerpt}
        descEn={seoEn.excerpt}
        descRu={seoRu.excerpt}
        descDe={seoDe.excerpt}
        ogType="article"
      ...
      <div className="container mx-auto px-4 md:px-6 py-10 max-w-3xl">
        ...
        <div className="flex items-center gap-5 mb-6 text-white/30 text-xs font-sans uppercase tracking-widest">
          ...
          <span className="flex items-center gap-1.5"><Clock size={12} /> {readLabel}</span>
        </div>
        ...
        {lang !== "tr" && t.blog.post.articleTurkishNotice ? (
          <p className="text-xs text-white/35 mb-8 border border-white/10 bg-white/[0.03] px-4 py-3 rounded-sm font-sans leading-relaxed">
            {t.blog.post.articleTurkishNotice}
          </p>
        ) : null}
        ...
        {(() => {
          ...
          return (
            <div className="mt-12 border-t border-white/8 pt-8">
              <h2 className="text-lg font-bold text-white mb-6 font-display">{t.blog.post.relatedTitle}</h2>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {related.map((r) => {
                  const rloc = getLocalizedBlogCard(r, lang);
                  return (
                  <Link key={r.id} href={`/blog/${r.slug}`}>
                    <article className="group border border-white/6 bg-[#151414] overflow-hidden hover:border-accent/25 transition-all duration-200 cursor-pointer">
                      <div className="relative h-28 overflow-hidden">
                        <img
                          src={r.image}
                          alt={`${rloc.title} — ChapterLOG blog`}
                      ...
                      <div className="p-3">
                        <span className={`text-[9px] font-bold px-2 py-0.5 font-sans ${r.tagColor}`}>{rloc.tag}</span>
